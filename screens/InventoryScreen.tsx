  ...
  return (
    <div className="bg-jewel-bg-dark p-4 rounded-lg shadow-md flex space-x-4 items-center">
      ...
      <div className="flex-grow">
        ...
        <p className={`text-sm font-semibold ${isLowStock ? 'text-yellow-400' : isOutOfStock ? 'text-jewel-red' : 'text-gray-400'}`}>
      ...
      <div className="flex flex-col items-end justify-between self-stretch">
        ...
        <div className="flex items-center space-x-2">
            <button onClick={onEdit} className="p-1 text-gray-400 hover:text-white"><EditIcon className="h-4 w-4" /></button>
            <button onClick={onDelete} className="p-1 text-gray-400 hover:text-jewel-red"><TrashIcon className="h-4 w-4" /></button>
        </div>
      ...
      <div className="space-y-4">
        {filteredProducts.length > 0 ? (
          ...
          <div className="bg-jewel-bg-dark p-8 rounded-lg text-center text-gray-400 border border-white/10 flex flex-col items-center">
            ...
            <p className="font-semibold text-white">No products found for this filter.</p>
            <p className="text-sm mt-1">Add a new product or change the filter.</p>
          </div>
        )}
      </div>

       <button onClick={handleOpenAddModal} className="fixed bottom-20 right-4 bg-jewel-gold text-jewel-navy h-14 w-14 rounded-full shadow-lg flex items-center justify-center">
